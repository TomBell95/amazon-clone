              ...
              <form>
                <div class="a-spacing-medium a-spacing-top-medium">
                  ...
                  <div class="a-spacing-top-medium">
                    ...
                    v-model="fullName" 
                    type="text" 
                    class="a-input-text" style="width: 100%;"
                  ...
                  <!-- Postode -->
                  <div class="a-spacing-top-medium">
                    <label style="margin-bottom: 0px;">Postcode</label>
                    <input 
                      v-model="postcode" 
                      type="text" 
                      class="a-input-text" 
...
export default {
    async asyncData({ $axios, params }) {
        try {
            ...
            const singleAddress = await $axios.$get(`api/addresses/${params.id}`);

            const [countriesResponse, addressResponse] = await Promise.all([
                response, singleAddress
            ])

            return { countries: countriesResponse, address: addressResponse.address }
    ...
    methods: {
        async onUpdateAddress(){
            try {
                ...
                console.log(data);
                const response = await this.$axios.$put('/api/addresses', data);
